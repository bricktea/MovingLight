...
bool Config::isOffhandItem(const HashedString& name) {
  return std::find(mOffHandItems.begin(), mOffHandItems.end(), name) !=
         mOffHandItems.end();
}
...
void Config::_computeLightBlocks() {
  ...
  if (afterLightBlocks - beforeLightBlocks > 0) {
    logger.warn("{} new glowing block(s) have been discovered!",
                afterLightBlocks - beforeLightBlocks);
    _save();
  }
}
